...
def train_one_fold(fold_idx: int, train_idx: list, val_idx: list, cases: list, device: torch.device, data_config: dict,
    ...
    train_ds = ProstateDataset([cases[i] for i in train_idx], augment=True, modalities=data_config['modalities'],
                               use_torchio=use_torchio)
    val_ds = ProstateDataset([cases[i] for i in val_idx], augment=False, modalities=data_config['modalities'],
                             use_torchio=use_torchio)
    ...
    scheduler_config = training_config.get('scheduler',
                                           {'type': 'plateau', 'mode': 'min', 'factor': 0.5, 'patience': 3})
    ...
    with open(log_path, 'w', newline='') as f:
        ...
        if loss_type == 'dice_bce':
            writer_csv.writerow(
                ['epoch', 'train_loss', 'val_loss', 'val_dice', 'train_dice_loss', 'train_bce_loss', 'val_dice_loss',
                 'val_bce_loss'])
        else:
